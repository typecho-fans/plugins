...
Sitemap v1.0.7 - 社区维护版
======================
<h4 align="center">—— 默认动态生成，可选生成静态文件的 Xml 格式站点地图插件，支持输出分类/标签页地址。</h4>
...
##### 2. 激活插件，默认保持原有的动态生成模式；
##### 3. “站点地图文件名”同时控制动态路由和静态文件，默认是 `sitemap.xml`；
##### 4. 可配置“标签最少文章数”，设置为 `2` 可排除只有一篇文章的低内容标签页；
##### 5. 如需静态文件，在插件设置中选择“静态文件”并保存，插件会立即在站点根目录生成对应文件；
##### 6. 静态模式下发布、变更状态或删除文章及页面后会自动刷新文件；修改分类、标签或由外部程序直接修改数据库后，需要重新保存插件设置；
##### 7. 切回动态模式会删除插件生成的静态文件并恢复同名动态路由。

静态模式要求 PHP 对站点根目录具有写权限。若根目录已有同名且非本插件生成的 XML 文件，插件不会覆盖。
 ...
 * v1.0.7 (26-08-21 [@NHPT](https://github.com/NHPT))
   * 保持原有动态生成模式为默认选项；
   * 支持自定义站点地图文件名，动态路由和静态文件统一使用该配置；
   * 支持在插件设置中手动切换为静态文件，并在保存时生成到站点根目录；
   * 静态模式会注销动态路由并在内容变更后自动原子刷新，切回动态模式时恢复路由并移除插件生成的文件。
