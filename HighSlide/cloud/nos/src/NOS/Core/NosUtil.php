 ...
 * Class NOSUtil
 *
 * NOS工具类，主要供NOSClient使用，用户也可以使用本类进行返回结果的格式化
 *
 * @package NOS
 */
class NosUtil
{
    ...
    const NOS_CONTENT = 'content';
    const NOS_LENGTH = 'length';
    const NOS_HEADERS = 'headers';

    const NOS_MAX_PART_SIZE = 1048576000;
    // 100MB
    const NOS_MID_PART_SIZE = 52428800;
    // 50MB
    const NOS_MIN_PART_SIZE = 16384;
    // 16K

    /**
     * 生成query params
     *
     * @param array $options关联数组
     *
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && ! is_array($options)) {
            throw new NosException($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            throw new NosException("http body content is invalid");
        }
    }
     ...
     * @return void
     */
    public static function throwNOSExceptionWithMessageIfEmpty($name, $errMsg)
    {
        if (empty($name)) {
            throw new NosException($errMsg);
        }
    }
...
BBB;
    ...
    {
        $content_md5 = "";
        if (($to_pos - $from_pos) > self::NOS_MAX_PART_SIZE) {
            return $content_md5;
        }
    ...
    public static function createPutBucketDuplicationXmlBody($duplication)
    {
        $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="utf-8"?><DeduplicationConfiguration></DeduplicationConfiguration>');
        $xml->addChild('Status', $duplication);
        return trim($xml->asXML());
    }

    public static function createPutBucketVersingXmlBody($versing)
    {
        $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="utf-8"?><VersioningConfiguration></VersioningConfiguration>');
        $xml->addChild('Status', $versing);
        return trim($xml->asXML());
    }
     ...
     * @param unknown $key
     */
    public static function creatPutBucketDefault404XmlBody($key)
    {
        $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="utf-8"?><Default404Configuration></Default404Configuration>');
     ...
     * @return string
     */
    public static function createDeleteObjectsXmlBody($objects,$quite)
    {
        $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="utf-8"?><Delete></Delete>');
        $xml->addChild('Quiet', $quite);
        foreach ($objects as $object) {
            $sub_object = $xml->addChild('Object');
            $object = NosUtil::sReplace($object);
    ...
    */
    ...
    {
        return str_replace(array("+","%2A"),array("%20","*"), urlencode($object));
    }
}
