...
- 修改仓库内插件的版本或发布信息时，先更新插件头信息及插件目录内的 README，并同步更新根目录 `README.md` 中对应条目（至少版本号），以触发 GitHub Actions 重新打包和发布插件。
- `TESTORE.md`、外部插件条目及部分 `ZIP_CDN` 数据由 `AUTO-UPDATE.php` 和 GitHub Actions 自动维护，除新增或修正收录信息外无需顺手修改。
