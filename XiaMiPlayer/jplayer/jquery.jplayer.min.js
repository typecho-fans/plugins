...
(function($) {
/* For Plugin */
XiaMiPlayerInit = function(song, playbox, autoplay){
	var box = $('.play', playbox), board = $('span', playbox), progress = $('.progress', playbox);
	return box.jPlayer({
		ready: function() {
			box.jPlayer('setMedia', {
				mp3: song.attr('data-url'),
				ogg: song.attr('data-url'),
				wma: song.attr('data-url')
			});
			if(autoplay) {
				$('.btn-pause') && $('.btn-pause').click();
				$('.btn-play', playbox).click();
				$(this).jPlayer('play');
			}
			board.text(song.text());
			if(song.attr('data-xiami') === "1") {
				$.getJSON('http://songs.sinaapp.com/lyric.php?callback=?&url='+song.attr('data-lyric'), function(lrc) {
					var m = 0;
					box.unbind($.jPlayer.event.timeupdate).bind($.jPlayer.event.timeupdate, function(e) {
						if(m<lrc.length && e.jPlayer.status.currentTime>lrc[m][0])
							board.text(lrc[m][1]),m++;

						progress.width(Math.round(e.jPlayer.status.currentPercentAbsolute/100*board.width()));
					})
				})				
			}
		},
		ended: function() {
			var next = song.next().length > 0 ? song.next() : $('li:first-child', playbox);
			next.click();
		},
		preload: 'none',
		swfPath: 'http://songs.sinaapp.com/Jplayer.swf',
		supplied: 'mp3,ogg,wma',
		solution: "Microsoft Internet Explorer" == window.navigator.appName ? !0 : "Netscape" == window.navigator.appName && /trident\/\d\.\d/i.test(window.navigator.userAgent) ? 'flash, html' : 'html, flash',
		wmode: 'window'
	});
}
$(document).ready(function() {
	$('.xiamiplayer').on('click', '.btn-play', function() {
		$('.btn-pause') && $('.btn-pause').click();
		$(this).prev().jPlayer('play');
		$(this).removeClass('btn-play');
		$(this).addClass('btn-pause');		
	})
	$('.xiamiplayer').on('click', '.btn-pause', function() {
		$(this).prev().jPlayer('pause');
		$(this).removeClass('btn-pause');
		$(this).addClass('btn-play');
	})
	$('.xiamiplayer').on('click', 'li', function() {
		var playbox = $(this).parent().parent();
		$('.btn-pause', playbox).click() && $('.play', playbox).jPlayer('destroy');
		XiaMiPlayerInit($(this), playbox, true);
		$('.btn-play', playbox).click();
	})
